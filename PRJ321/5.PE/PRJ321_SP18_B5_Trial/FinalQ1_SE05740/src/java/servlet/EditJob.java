...
public class EditJob extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (title.trim().isEmpty() || title == null || company.trim().isEmpty()
                    || company == null || des.trim().isEmpty() || des == null) {
                request.setAttribute("error", "you must be fill all field");
                request.getRequestDispatcher("AddJob.jsp").forward(request, response);
            } else {
                JobDAO jd = new JobDAO();
                jd.update(Integer.parseInt(id), title, des, expi, des);
                response.sendRedirect(request.getContextPath() + "/ReListJob");
            }
        } catch (NumberFormatException e) {
            request.setAttribute("error", "year of experience must be number");
            request.getRequestDispatcher("AddJob.jsp").forward(request, response);
        }
    }
