    ...
    public Home getDetailsPost(int id) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String content = formatContent(rs.getString(6));
                String author = rs.getString(7);
                String createDate = formatDate("dd-MM-yyyy", rs.getString(8));
                home = new Home(id, type, title, imgLink, content, author, createDate);
            }
        } catch (Exception ex) {
            throw ex;
        } finally {
    ...
    public ArrayList<Home> getPost(int page, int pageSize) throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String title = rs.getString(3);
                String imgLink = db.getResource() + rs.getString(4);
                String content = formatContent(rs.getString(5)); // get short content
                String author = rs.getString(7);
                String createDate = formatDate("dd-MM-yyyy", rs.getString(8));
                String titleGroup = formatDate("MMMM yyyy", rs.getString(8));
                System.out.println(rs.getString(7));
                Home home = new Home(idPost, type, title, imgLink, content, author, createDate, titleGroup);
                homes.add(home);
            }
        } catch (Exception ex) {
            throw ex;
